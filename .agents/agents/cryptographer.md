...
**其他输入源**：
- 安全需求说明（来自用户或Architect，从task文件获取）
- `.agents/docs/architecture/` - 架构对加密模块的要求（从task文件获取路径）
...
**输出位置**：
- `.agents/docs/crypto-specs/{date}_cryptographer-{task-id}.md` - 加密方案文档
...
**详细日志位置**：`.agents/docs/logs/{task-id}/{date}_cryptographer-{task-id}.log`
